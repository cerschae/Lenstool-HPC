...
//
typedef void (*amplif_func_t)(type_t *ampli, const struct grid_param *frame, const struct Potential_SOA *lens, int Nlens, type_t dx, type_t dy, int nbgridcells_x, int nbgridcells_y, int istart, int jstart);
...
void amplif_grid_CPU(type_t *amplif, const struct grid_param *frame, const struct Potential_SOA *lens, int nhalos, int nbgridcells,int mode_amp, int istart, int jstart)
{
	type_t dx = (frame->xmax - frame->xmin)/(nbgridcells - 1);
	type_t dy = (frame->ymax - frame->ymin)/(nbgridcells - 1);
	//

	(*amplif_func[mode_amp])(amplif, frame, lens, nhalos, dx, dy, nbgridcells, nbgridcells, istart, jstart);
}
//
//table for ampli methods
//
void amplif_5_grid_general_CPU(type_t *ampli, const struct grid_param *frame, const struct Potential_SOA *lens, int Nlens, type_t dx, type_t dy, int nbgridcells_x, int nbgridcells_y, int istart, int jstart)
{
