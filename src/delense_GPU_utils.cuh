...
static
inline
void mychi_transformImageToSourcePlane_SOA_Packed_GPU( const struct point *image_point, double dlsds, struct point *source_point, double *grad_x, double * grad_y, int grad_id)
{
        ...
        //if (world_rank == 1)
        //printf("      %f %f = %f %f - dlsds = %f grad id = %d grad = (%f %f)\n", source_point->x, source_point->y,image_point->x, image_point->y, dlsds, grad_id, grad_x[grad_id], grad_y[grad_id]);
...
static
inline
void mychi_transformtriangleImageToSourcePlane_SOA_grid_gradient_upper_GPU( struct triplet *I, double dlsds, struct triplet *S, double *grad_x, double * grad_y, int grad_id, int nbgridcell)
{
        mychi_transformImageToSourcePlane_SOA_Packed_GPU( &I->a, dlsds,   &S->a, grad_x, grad_y, grad_id             );
        mychi_transformImageToSourcePlane_SOA_Packed_GPU( &I->b, dlsds,   &S->b, grad_x, grad_y, grad_id + nbgridcell);
        mychi_transformImageToSourcePlane_SOA_Packed_GPU( &I->c, dlsds,   &S->c, grad_x, grad_y, grad_id +          1);
}
//
__device__
static
inline
void mychi_transformtriangleImageToSourcePlane_SOA_grid_gradient_lower_GPU( struct triplet *I, double dlsds, struct triplet *S, double*
grad_x, double * grad_y, int grad_id, int nbgridcell)
{
        mychi_transformImageToSourcePlane_SOA_Packed_GPU( &I->a, dlsds,   &S->a, grad_x, grad_y, grad_id + nbgridcell + 1);
        mychi_transformImageToSourcePlane_SOA_Packed_GPU( &I->b, dlsds,   &S->b, grad_x, grad_y, grad_id +              1);
        mychi_transformImageToSourcePlane_SOA_Packed_GPU( &I->c, dlsds,   &S->c, grad_x, grad_y, grad_id + nbgridcell    );
}
...
static
inline
double mychi_determinant_GPU(const struct point *A,
...
static
inline
int mychi_inside_GPU(const struct point *P, struct triplet *T)
{
        double  s, s1, s2, d;

        d  = mychi_determinant_GPU(&T->a, &T->b, &T->c);
        s  = mychi_determinant_GPU(&T->a, &T->b, P)*d;
        if (s < 0.) return 0;
        s1 = mychi_determinant_GPU(&T->b, &T->c, P)*d;
        if (s1 < 0.) return 0;
        s2 = mychi_determinant_GPU(&T->c, &T->a, P)*d;
        if (s2 < 0.) return 0;
        return 1;
...
static
inline
int mychi_insideborder_GPU(const struct point *P, struct triplet *T)
{
        double  s, s1, s2, d;
...
static
inline
struct  point   mychi_barycenter_GPU(struct triplet *A)
{
        struct  point   B;
...
static
inline
double mychi_dist_GPU(struct point A, struct point B)
{
        double  x, y;
        x = A.x - B.x;
        y = A.y - B.y;
