...
void chi_bruteforce_SOA_CPU_grid_srcplane(double *chi, int *error, runmode_param *runmode, const struct Potential_SOA *lens, const struct grid_param *frame, const int *nimages_strongLensing, galaxy *images){
  ...
  double *grid_srcplane_x, *grid_srcplane_y;

  grid_srcplane_x = (double *)malloc((int) (runmode->nbgridcells) * (runmode->nbgridcells) * sizeof(double));
  grid_srcplane_y = (double *)malloc((int) (runmode->nbgridcells) * (runmode->nbgridcells) * sizeof(double));
  grid_dim = runmode->nbgridcells;
  //Packaging the image to sourceplane conversion
  grid_srcplane_conversion_CPU(grid_srcplane_x,grid_srcplane_y,frame,lens,images[0].dr,runmode->Nlens,grid_dim);
    ...
    for( int image_id = 0; image_id < nimages_strongLensing[source_id]; image_id++){
      ...
      for (int x_id = 0; x_id < runmode->nbgridcells-1; ++x_id ){
        for (int y_id = 0; y_id < runmode->nbgridcells-1; ++y_id ){
          ...
          chi_transformtriangleImageToSourcePlane_SOA_Packed_upper(runmode->Nlens,&Tsup, &Tsupsource,grid_srcplane_x,grid_srcplane_y,x_id*grid_dim+y_id, grid_dim);
          chi_transformtriangleImageToSourcePlane_SOA_Packed_lower(runmode->Nlens,&Tinf, &Tinfsource,grid_srcplane_x,grid_srcplane_y,x_id*grid_dim+y_id, grid_dim);
          ...
          if(chi_insideborder(&sources[source_id].center,&Tsupsource)==1){
            ...
            for(int i=1; i<nimages_strongLensing[source_id]; i++){  // get the distance to each real image and keep the index of the closest real image
              ...
              //printf(" im_index %d im pos x %f y %f xS %f yS %f im_dist actual %f im_dist %f \n",im_index,im_position.x,im_position.y,chi_barycenter(&Tsupsource).x,chi_barycenter(&Tsupsource).y, im_dist[im_index], im_dist[i]);
            }
          }
          ...
          if(chi_inside(&sources[source_id].center,&Tinfsource)==1){
            ...
            for(int i=1; i<nimages_strongLensing[source_id]; i++){  // get the distance to each real image and keep the index of the closest real image
              ...
              //printf(" im_index %d im pos x %f y %f xS %f yS %f im_dist actual %f im_dist %f \n",im_index,im_position.x,im_position.y,chi_barycenter(&Tinfsource).x,chi_barycenter(&Tinfsource).y, im_dist[im_index], im_dist[i]);
            }
          }
...
void chi_bruteforce_SOA_CPU_grid_gradient(double *chi, int *error, runmode_param *runmode, const struct Potential_SOA *lens, const struct grid_param *frame, const int *nimages_strongLensing, galaxy *images){
  ...
  grid_dim = runmode->nbgridcells;
  //Packaging the image to sourceplane conversion
  gradient_grid_CPU(grid_gradient_x,grid_gradient_y,frame,lens,runmode->Nlens,grid_dim);
    ...
    for( int image_id = 0; image_id < nimages_strongLensing[source_id]; image_id++){
      ...
      for (int x_id = 0; x_id < runmode->nbgridcells-1; ++x_id ){
        for (int y_id = 0; y_id < runmode->nbgridcells-1; ++y_id ){
          ...
          chi_transformtriangleImageToSourcePlane_SOA_grid_gradient_upper(runmode->Nlens,&Tsup, sources[source_id].dr,&Tsupsource,grid_gradient_x,grid_gradient_y,x_id*grid_dim+y_id, grid_dim);
          chi_transformtriangleImageToSourcePlane_SOA_grid_gradient_lower(runmode->Nlens,&Tinf, sources[source_id].dr, &Tinfsource,grid_gradient_x,grid_gradient_y,x_id*grid_dim+y_id, grid_dim);
...
void chi_bruteforce_SOA_CPU_grid_gradient_unsorted(double *chi, int *error, runmode_param *runmode, const struct Potential_SOA *lens, const struct grid_param *frame, const int *nimages_strongLensing, galaxy *images){
  ...
  grid_dim = runmode->nbgridcells;
  //Packaging the image to sourceplane conversion
  gradient_grid_CPU_unsorted(grid_gradient_x,grid_gradient_y,frame,lens,runmode->nhalos,grid_dim);
  ...
  *chi = 0;
  ...
  for( int  source_id = 0; source_id < runmode->nsets; source_id ++){
    ...
    for( int image_id = 0; image_id < nimages_strongLensing[source_id]; image_id++){
      ...
      //////////////////////////////////////computation of theoretical sources//////////////////////////////////////
      chi_transformImageToSourcePlane_SOA_unsorted(runmode->nhalos, &images[index+image_id].center,images[index+image_id].dr,lens,&sources[source_id].center);
...
{   // dlsds is the distance between lens and source divided by the distance observer-source
    struct point Grad;  // gradient
    Grad = module_potentialDerivatives_totalGradient_SOA(image_point, &lens[1], Nlens[1]);
...
void chi_transformImageToSourcePlane_SOA_unsorted(const int Nlens, const struct point *image_point, double dlsds, const struct Potential_SOA *lens, struct point *source_point)
{   // dlsds is the distance between lens and source divided by the distance observer-source
    struct point Grad;  // gradient
...
*
*/
void chi_transformtriangleImageToSourcePlane_SOA_Packed_upper(const int *Nlens, struct triplet *I, struct triplet *S, double *grid_srcplane_x, double * grid_srcplane_y, int grad_id, int nbgridcell)
{
	S->a.x = grid_srcplane_x[grad_id];
	S->a.y = grid_srcplane_y[grad_id];
	S->b.x = grid_srcplane_x[grad_id+1];
	S->b.y = grid_srcplane_y[grad_id+1];
	S->c.x = grid_srcplane_x[grad_id+nbgridcell];
	S->c.y = grid_srcplane_y[grad_id+nbgridcell];

}

void chi_transformtriangleImageToSourcePlane_SOA_Packed_lower(const int *Nlens, struct triplet *I, struct triplet *S, double *grid_srcplane_x, double * grid_srcplane_y, int grad_id, int nbgridcell)
{

	S->a.x = grid_srcplane_x[grad_id+nbgridcell+1];
	S->a.y = grid_srcplane_y[grad_id+nbgridcell+1];
	S->b.x = grid_srcplane_x[grad_id+nbgridcell];
	S->b.y = grid_srcplane_y[grad_id+nbgridcell];
	S->c.x = grid_srcplane_x[grad_id+1];
	S->c.y = grid_srcplane_y[grad_id+1];
}
...
{
  chi_transformImageToSourcePlane_SOA_Packed(Nlens, &I->a, dlsds,   &S->a, grad_x, grad_y, grad_id);
  chi_transformImageToSourcePlane_SOA_Packed(Nlens, &I->b, dlsds,   &S->b, grad_x, grad_y, grad_id+1);
  chi_transformImageToSourcePlane_SOA_Packed(Nlens, &I->c, dlsds,   &S->c, grad_x, grad_y, grad_id+nbgridcell);
}
...
{
  chi_transformImageToSourcePlane_SOA_Packed(Nlens, &I->a, dlsds,   &S->a, grad_x, grad_y, grad_id+nbgridcell+1);
  chi_transformImageToSourcePlane_SOA_Packed(Nlens, &I->b, dlsds,   &S->b, grad_x, grad_y, grad_id+nbgridcell);
  chi_transformImageToSourcePlane_SOA_Packed(Nlens, &I->c, dlsds,   &S->c, grad_x, grad_y, grad_id+1);
}
